      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 12 }}>
            <div className="skeleton" style={{ width: 32, height: 32, borderRadius: '50%', background: '#1c2636' }} />
            <p style={{ color: '#484f58', fontSize: 13 }}>加载中...</p>
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 12 }}>
            ...
            <div key={conv.id} onClick={() => navigate(`/messages/${conv.id}`)}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                  ...
                  <span style={{ color: '#484f58', fontSize: 11, flexShrink: 0 }}>{formatTime(conv.lastTime)}</span>
                </div>
